...
Granting bypass to a GitHub App is not enough on its own: the workflow authenticates as `github-actions[bot]` through `GITHUB_TOKEN`, so the push is still rejected even when the app is listed as a bypass actor. The push has to authenticate as the app. Mint an installation token with `actions/create-github-app-token`, pass it to `actions/checkout` as `token:`, and use it as `GITHUB_TOKEN` for semantic-release. `references/release-workflow.yml` carries that step commented out; uncomment it and swap both token references. The app id belongs in a repository variable and the private key in a secret, and the app needs contents and pull-requests write on the repository.
